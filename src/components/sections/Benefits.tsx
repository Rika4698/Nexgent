...
export default function Benefits() {
  return (
    <section className="bg-slate-100 font-sans py-20 px-6 text-center">
      <motion.span
        variants={fadeUp}
        initial="hidden"
        whileInView="visible"
      ...
      >
        Why Choose Us To{" "}<br />
        <span className="font-serif inline-block transform -skew-x-12 font-normal">Grow Smarter</span>
      </motion.h2>
      ...
      <div className="mt-16 max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {benefits.map((benefit, i) => (
          <motion.div
            ...
            className="group bg-white  rounded-3xl p-8  text-left  transition-shadow duration-300"
          >
            ...
            <p className="text-[#5D5D5D] text-lg font-medium leading-relaxed tracking-tight">{benefit.description}</p>
          </motion.div>
        ))}
      </div>
    </section>
